...
export default class NewJob extends React.Component {
  ...
  render() {
    ...
    return (
      <form
        ref={c => { this.form = c }} action="/jobs" acceptCharset="UTF-8" method="post"
        ...
        <input
          ...
          className={this.fieldClasses('title')}
          name="job[title]"
        ...
        <input
          ...
          className={this.fieldClasses('company')}
          name="job[company]"
        ...
        <input
          ...
          className={this.fieldClasses('location')}
          name="job[location]"
          placeholder="Chicago, Il" />

        <label htmlFor="source">Link to your job description</label>
        <input
          id="source"
          type="text"
          value={this.state.source}
          onChange={e => this.handleChange('source', e)}
          className={this.fieldClasses('source')}
          name="job[source]"
          placeholder="https://acme.inc/jobs/78" />
        ...
        <input
          ...
          onChange={e => this.handleChange('company_url', e)}
          className={this.fieldClasses('company_url')}
        ...
        <div className="clearfix">
          <div className="col col-8">
            ...
            <input
              ...
              onChange={e => this.handleChange('company_logo', e)}
              className={this.fieldClasses('company_logo')}
              name="job[company_logo]"
              placeholder="https://acme.inc/logo.png" />
          </div>

          <div className="col col-4 px2">
            <img
              alt="Company Logo" src={this.state.validLogoUrl}
              style={{ width: 200, height: 200, objectFit: 'cover' }} />
          </div>
        </div>

        <label htmlFor="author_name">Contact Name</label>
        <input
          id="author_name"
          type="text"
          value={this.state.author_name}
          onChange={e => this.handleChange('author_name', e)}
          className={this.fieldClasses('author_name')}
        ...
        <input
          ...
          onChange={e => this.handleChange('author_email', e)}
          className={this.fieldClasses('author_email')}
          name="job[author_email]"
          placeholder="Your email for the receipt" />

        <div className="col-12">
          <input
            id="role_type_full_time"
            name="job[role_type]" type="radio" defaultValue="Full Time" defaultChecked />
          <label htmlFor="role_type_full_time">Full Time</label>
          <input
            id="role_type_part_time"
            name="job[role_type]" type="radio" defaultValue="Part Time" />
          <label htmlFor="role_type_part_time">Part Time</label>
          <input
            id="role_type_part_time"
            name="job[role_type]" type="radio" defaultValue="Contract" />
  ...
  handleBlur(e) {
    ...
    const field = match[1]
    if (field && njRequiredFields.indexOf(field) !== -1) {
      if (!this.state[field]) {
  ...
  validateFields() {
    let brokenFields = njRequiredFields.filter(f => !this.state[f])
